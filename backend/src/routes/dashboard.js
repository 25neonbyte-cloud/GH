...
async function calculate() {
  ...
  const [totalLeitos, leitosLivres, leitosOcupados, internacoes, internacoesHoje, altasHoje] = await Promise.all([
    ...
    prisma.internacao.count({ where: { createdAt: { gte: today, lt: tomorrow } } }),
  ...
  return { taxaOcupacao: totalLeitos ? Number(((leitosOcupados/totalLeitos)*100).toFixed(1)) : 0, totalLeitos, leitosLivres, leitosOcupados, internacoesAtivas: internacoes.length, mediaLOS: Number(mediaLOS.toFixed(1)), internacoesHoje, altasHoje, ultimaAtualizacao: new Date() };
}
...
router.get('/ocupacao-7dias', async (req,res)=>{
  const result=[];
  for(let n=6;n>=0;n--){
    const start=new Date(); start.setHours(0,0,0,0); start.setDate(start.getDate()-n); const end=new Date(start); end.setDate(start.getDate()+1);
