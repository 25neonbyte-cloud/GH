...
function EvolutionEvent({item}) {
  ...
  return <div className="card p-4">
    ...
    <div className="grid md:grid-cols-2 gap-x-5 gap-y-2 mt-3 text-sm">
      {Object.entries(content).filter(([,v])=>v!==null&&v!==undefined&&v!=='').map(([k,v])=><div key={k}><span className="text-slate-500">{k.replace(/([A-Z])/g,' $1').replace(/^./,s=>s.toUpperCase())}: </span><b className="whitespace-pre-wrap">{String(v)}</b></div>)}
    </div>
    {!Object.keys(content).length&&<p className="mt-3 whitespace-pre-wrap">{e.observacoes}</p>}
    {!!e.camposHerdados?.length&&<div className="mt-3 text-xs text-amber-700">Campos confirmados a partir da evolução anterior: {e.camposHerdados.join(', ')}</div>}
  </div>;
}
